...
class NovaWeb3Provider extends EventEmitter {
  ...
  sendNullResponse(id) {
    ...
    if (this.isDebug) {
      console.log(
        `<== sendResponse id: ${id}, data: ${JSON.stringify(data)}`
      );
    }
    if (callback) {
      wrapResult ? callback(null, data) : callback(null, null);
      this.callbacks.delete(id);
    } else {
      console.log(`callback id: ${id} not found`);
      // check if it's iframe callback
      for (var i = 0; i < window.frames.length; i++) {
        ...
        try {
          if (frame.ethereum.callbacks.has(id)) {
            frame.ethereum.sendResponse(id, null);
          }
        } catch (error) {
          console.log(`send response to frame error: ${error}`);
        }
  ...
  sendRpcError(id, code, message) {
     let error = new ProviderRpcError(code, message);
